...
names(tab)=unique(objres$data$Tp)
...
par(mar = c(4, 4, 1, 1))
    ...
    names(tmpbxp)[2]="x"
  re=boxplot(x~Id,tmpbxp,axes=F,cex=0,ylab="Pearson residuals",ylim=range(plotdata$limxqq))
  abline(h=0)
   axis(2,at=plotdata$limxqq,las=2)
   for(i in 1:length(re$n)) axis(1,at=i,re$name[i],las=2,tick=F,ifelse(length(re$names)>5,0.6,1))
  beeswarm(x~Id,tmpbxp,add=T,pwcol=tmpbxp$color,pch=16)
```
